...
#     int_value = int(int_input.value)  # Get the integer value
#     string_value = string_input.value  # Get the string value
#     datetime_value = date_input.value  # Get the datetime value

#     stock_analysis(string_value, datetime_value.strftime(
#         '%Y-%m-%d %H:%M:%S'), int_value)


# # Create widgets
# int_input = widgets.IntText(value=6, description='Months:')
# string_input = widgets.Text(value='AAPL', description='Stock:')
# date_input = widgets.DatePicker(
#     value=datetime.today(), description='Buy date (optional):')
...
# # Display the widgets
# display(int_input, string_input, date_input, run_button)

st.title("📈 Stock Analysis Tool")

stock = st.text_input("Enter Stock Symbol", "AAPL")
buy_date = st.date_input("Buy Date", datetime.today())
months = st.number_input("Months of Data", min_value=1, max_value=60, value=6)

if st.button("Show Analysis"):
    stock_analysis(stock, buy_date.strftime('%Y-%m-%d'), months)
